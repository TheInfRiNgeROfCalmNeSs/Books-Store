...
class BookStore extends PureComponent {
  ...
  getSnapshotBeforeUpdate(prevProps, prevState) {
    ...
    if(this.state.timerId===null && this.state.timer===60*0.2) {
      if(this.state.step > 1 && this.state.step < 5) {
        this.setState({timerId: setInterval(() => this.setState({timer: this.state.timer - 1}), 1000)})
      }
    }
    ...
    if(prevState.modal !== this.state.modal && prevState.timer===0) {
      this.setState({ modal: true, timer: 60*0.2 })
    }
    if(this.state.verticalPage && this.state.step === 1 && prevState.step < 1) {
  ...
  componentDidUpdate(props, prevState, varFromGetSnap) {
    /*if(prevState.timer===0) {
      this.setState({timer: 60*0.2})
    }*/
    if(varFromGetSnap==="lets hide loader") {
      this.setState({noThumb: {"no": null}})
    }
    if(varFromGetSnap==="vertical page") {
      window.scrollTo({left: 0, top: 366+(181*(this.state.verticalPage-1)), behavior: 'smooth' })
    }
  ...
  render() {
    const { selectedBooks, step, error, fullName, contactNumber, shippingAddress, deliveryOption, timer, timerId, searching, searchCompleted, start, numFound, docs, loading, page, thumbs, noThumb, checkedBooks, prevPage, searchTerm, verticalPage } = this.state
    ...
    return (
      <Fragment>
        ...
        <div className="App">
          {
            step === 1 ?
              <BookSearch updateFormData={this.updateFormData} selectedBooks={selectedBooks} searching={searching} searchCompleted={searchCompleted}
                          error={error} start={start} numFound={numFound} docs={docs} fullName={fullName}
                          loading={loading} page={page} thumbs={thumbs} checkedBooks={checkedBooks} prevPage={prevPage} searchTerm={searchTerm}
                          verticalPage={verticalPage} />
            :
            step === 2 ?
            ...
            :
              <BookSearch updateFormData={this.updateFormData} selectedBooks={selectedBooks} searching={searching} searchCompleted={searchCompleted} error={error} start={start} numFound={numFound} docs={docs} fullName={fullName} loading={loading} page={page} thumbs={thumbs} />
          }
          {
            step > 1 && step < 5 ?
              <Alert color="warning">{`You have ${minutes} Minutes, ${seconds} Seconds, before confirming order`}</Alert>
            :
            step === 1 && timerId === null && timer===0 ?
  ...
  updateFormData = (formData) => {
    const { step, selectedBooks, error, fullName, contactNumber, shippingAddress, deliveryOption, timer, timerId, docs, numFound, start, searchCompleted, searching, page, thumbs, noThumb, checkedBooks, prevPage, searchTerm, verticalPage } = formData
    ...
    this.setState({
      ...
      timer: timer===0?60*0.2:this.state.timer,
      ...
      verticalPage: verticalPage!==undefined?verticalPage:this.state.verticalPage,
    });
  }
